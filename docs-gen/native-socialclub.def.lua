--- @meta

--- ScInboxGetTotalNumMessages
---
--- @hash [0x03A93FF1A2CA0864](https://docs.fivem.net/natives/?_0x03A93FF1A2CA0864)
---
--- @return number
--- @overload fun(): number
function ScInboxGetTotalNumMessages() end

    
--- # New Name: ScInboxGetTotalNumMessages
--- ScInboxGetTotalNumMessages
---
--- @hash [0x03A93FF1A2CA0864](https://docs.fivem.net/natives/?_0x03A93FF1A2CA0864)
---
--- @return number
--- @overload fun(): number
--- @deprecated
function N_0x03a93ff1a2ca0864() end

    
--- # New Name: ScInboxGetTotalNumMessages
--- ScInboxGetTotalNumMessages
---
--- @hash [0x03A93FF1A2CA0864](https://docs.fivem.net/natives/?_0x03A93FF1A2CA0864)
---
--- @return number
--- @overload fun(): number
--- @deprecated
function GetTotalScInboxIds() end

    
--- ScInboxGetEmails
---
--- @hash [0x040ADDCBAFA1018A](https://docs.fivem.net/natives/?_0x040ADDCBAFA1018A)
--- @param offset number (int)
--- @param limit number (int)
--- @return nil
--- @overload fun(offset: number, limit: number): nil
function ScInboxGetEmails(offset, limit) end

    
--- # New Name: ScInboxGetEmails
--- ScInboxGetEmails
---
--- @hash [0x040ADDCBAFA1018A](https://docs.fivem.net/natives/?_0x040ADDCBAFA1018A)
--- @param offset number (int)
--- @param limit number (int)
--- @return nil
--- @overload fun(offset: number, limit: number): nil
--- @deprecated
function N_0x040addcbafa1018a(offset, limit) end

    
--- Returns true if the add license plate text request is still pending.
---
--- @hash [0x07C61676E5BB52CD](https://docs.fivem.net/natives/?_0x07C61676E5BB52CD)
--- @param token number (int)
--- @return boolean
--- @overload fun(token: number): boolean
function ScLicenseplateGetAddIsPending(token) end

    
--- # New Name: ScLicenseplateGetAddIsPending
--- Returns true if the add license plate text request is still pending.
---
--- @hash [0x07C61676E5BB52CD](https://docs.fivem.net/natives/?_0x07C61676E5BB52CD)
--- @param token number (int)
--- @return boolean
--- @overload fun(token: number): boolean
--- @deprecated
function N_0x07c61676e5bb52cd(token) end

    
--- N_0x07dbd622d9533857
---
--- @hash [0x07DBD622D9533857](https://docs.fivem.net/natives/?_0x07DBD622D9533857)
--- @param p0 any
--- @return any
--- @overload fun(p0: any): any
function N_0x07dbd622d9533857(p0) end

    
--- SC native to start a request for if license plate text is valid
---
--- @hash [0x0F73393BAC7E6730](https://docs.fivem.net/natives/?_0x0F73393BAC7E6730)
--- @param plateText string (char*)
--- @param tokenOut number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
function ScLicenseplateIsvalid(plateText, tokenOut) end

    
--- # New Name: ScLicenseplateIsvalid
--- SC native to start a request for if license plate text is valid
---
--- @hash [0x0F73393BAC7E6730](https://docs.fivem.net/natives/?_0x0F73393BAC7E6730)
--- @param plateText string (char*)
--- @param tokenOut number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
--- @deprecated
function N_0x0f73393bac7e6730(plateText, tokenOut) end

    
--- N_0x116fb94dc4b79f17
---
--- @hash [0x116FB94DC4B79F17](https://docs.fivem.net/natives/?_0x116FB94DC4B79F17)
--- @param p0 string (char*)
--- @return nil
--- @overload fun(p0: string): nil
function N_0x116fb94dc4b79f17(p0) end

    
--- N_0x16da8172459434aa
---
--- @hash [0x16DA8172459434AA](https://docs.fivem.net/natives/?_0x16DA8172459434AA)
---
--- @return any
--- @overload fun(): any
function N_0x16da8172459434aa() end

    
--- ScProfanityGetCheckIsValid
---
--- @hash [0x1753344C770358AE](https://docs.fivem.net/natives/?_0x1753344C770358AE)
--- @param token number (int)
--- @return boolean
--- @overload fun(token: number): boolean
function ScProfanityGetCheckIsValid(token) end

    
--- # New Name: ScProfanityGetCheckIsValid
--- ScProfanityGetCheckIsValid
---
--- @hash [0x1753344C770358AE](https://docs.fivem.net/natives/?_0x1753344C770358AE)
--- @param token number (int)
--- @return boolean
--- @overload fun(token: number): boolean
--- @deprecated
function N_0x1753344c770358ae(token) end

    
--- # New Name: ScProfanityGetCheckIsValid
--- ScProfanityGetCheckIsValid
---
--- @hash [0x1753344C770358AE](https://docs.fivem.net/natives/?_0x1753344C770358AE)
--- @param token number (int)
--- @return boolean
--- @overload fun(token: number): boolean
--- @deprecated
function ScHasCheckStringTaskCompleted(token) end

    
--- N_0x19853b5b17d77bca
---
--- @hash [0x19853B5B17D77BCA](https://docs.fivem.net/natives/?_0x19853B5B17D77BCA)
--- @param p0 any
--- @param p1 any
--- @return boolean
--- @overload fun(p0: any): boolean, any
function N_0x19853b5b17d77bca(p0, p1) end

    
--- Social Club native to add license plate for the user
---
--- @hash [0x1989C6E6F67E76A8](https://docs.fivem.net/natives/?_0x1989C6E6F67E76A8)
--- @param plateText string (char*)
--- @param plateData string (char*)
--- @param tokenOut number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, string): boolean, number
function ScLicenseplateAdd(plateText, plateData, tokenOut) end

    
--- # New Name: ScLicenseplateAdd
--- Social Club native to add license plate for the user
---
--- @hash [0x1989C6E6F67E76A8](https://docs.fivem.net/natives/?_0x1989C6E6F67E76A8)
--- @param plateText string (char*)
--- @param plateData string (char*)
--- @param tokenOut number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, string): boolean, number
--- @deprecated
function N_0x1989c6e6f67e76a8(plateText, plateData, tokenOut) end

    
--- ```
--- Returns the nickname of the logged-in Rockstar Social Club account.
--- ```
---
--- @hash [0x198D161F458ECC7F](https://docs.fivem.net/natives/?_0x198D161F458ECC7F)
---
--- @return string
--- @overload fun(): string
function ScGetNickname() end

    
--- ```
--- NativeDB Added Parameter 1: char* p0
--- NativeDB Added Parameter 2: char* p1
--- NativeDB Added Parameter 3: char* p2
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0x1D12A56FC95BE92E](https://docs.fivem.net/natives/?_0x1D12A56FC95BE92E)
---
--- @return boolean
--- @overload fun(): boolean
function N_0x1d12a56fc95be92e() end

    
--- Gets license plate text related to the get license plate info request
---
--- @hash [0x1D4446A62D35B0D0](https://docs.fivem.net/natives/?_0x1D4446A62D35B0D0)
--- @param token number (int)
--- @param plateIndex number (int)
--- @return string
--- @overload fun(token: number, plateIndex: number): string
function ScLicenseplateGetPlate(token, plateIndex) end

    
--- ScPresenceAttrSetInt
---
--- @hash [0x1F1E9682483697C7](https://docs.fivem.net/natives/?_0x1F1E9682483697C7)
--- @param attrHash Hash
--- @param value number (int)
--- @return boolean
--- @overload fun(attrHash: Hash, value: number): boolean
function ScPresenceAttrSetInt(attrHash, value) end

    
--- # New Name: ScPresenceAttrSetInt
--- ScPresenceAttrSetInt
---
--- @hash [0x1F1E9682483697C7](https://docs.fivem.net/natives/?_0x1F1E9682483697C7)
--- @param attrHash Hash
--- @param value number (int)
--- @return boolean
--- @overload fun(attrHash: Hash, value: number): boolean
--- @deprecated
function N_0x1f1e9682483697c7(attrHash, value) end

    
--- N_0x225798743970412b
---
--- @hash [0x225798743970412B](https://docs.fivem.net/natives/?_0x225798743970412B)
--- @param p0 number (int*)
--- @return boolean
--- @overload fun(): boolean, number
function N_0x225798743970412b(p0) end

    
--- ScEmailMessagePushGamerToRecipList
---
--- @hash [0x2330C12A7A605D16](https://docs.fivem.net/natives/?_0x2330C12A7A605D16)
--- @param networkHandle number (int*)
--- @return nil
--- @overload fun(): nil, number
function ScEmailMessagePushGamerToRecipList(networkHandle) end

    
--- ```
--- NativeDB Added Parameter 1: char* p0
--- NativeDB Added Parameter 2: float* p1
--- NativeDB Added Parameter 3: char* p2
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0x2570E26BE63964E3](https://docs.fivem.net/natives/?_0x2570E26BE63964E3)
---
--- @return boolean
--- @overload fun(): boolean
function N_0x2570e26be63964e3() end

    
--- ScPresenceAttrSetString
---
--- @hash [0x287F1F75D2803595](https://docs.fivem.net/natives/?_0x287F1F75D2803595)
--- @param attrHash Hash
--- @param value string (char*)
--- @return boolean
--- @overload fun(attrHash: Hash, value: string): boolean
function ScPresenceAttrSetString(attrHash, value) end

    
--- # New Name: ScPresenceAttrSetString
--- ScPresenceAttrSetString
---
--- @hash [0x287F1F75D2803595](https://docs.fivem.net/natives/?_0x287F1F75D2803595)
--- @param attrHash Hash
--- @param value string (char*)
--- @return boolean
--- @overload fun(attrHash: Hash, value: string): boolean
--- @deprecated
function N_0x287f1f75d2803595(attrHash, value) end

    
--- ScInboxMessagePop
---
--- @hash [0x2C015348CF19CA1D](https://docs.fivem.net/natives/?_0x2C015348CF19CA1D)
--- @param p0 number (int)
--- @return boolean
--- @overload fun(p0: number): boolean
function ScInboxMessagePop(p0) end

    
--- # New Name: ScInboxMessagePop
--- ScInboxMessagePop
---
--- @hash [0x2C015348CF19CA1D](https://docs.fivem.net/natives/?_0x2C015348CF19CA1D)
--- @param p0 number (int)
--- @return boolean
--- @overload fun(p0: number): boolean
--- @deprecated
function N_0x2c015348cf19ca1d(p0) end

    
--- ```
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0x2D874D4AE612A65F](https://docs.fivem.net/natives/?_0x2D874D4AE612A65F)
---
--- @return any
--- @overload fun(): any
function N_0x2d874d4ae612a65f() end

    
--- Gets license plate data related to the get license plate info request
---
--- @hash [0x2E89990DDFF670C3](https://docs.fivem.net/natives/?_0x2E89990DDFF670C3)
--- @param token number (int)
--- @param plateIndex number (int)
--- @return string
--- @overload fun(token: number, plateIndex: number): string
function ScLicenseplateGetPlateData(token, plateIndex) end

    
--- # New Name: ScLicenseplateGetPlateData
--- Gets license plate data related to the get license plate info request
---
--- @hash [0x2E89990DDFF670C3](https://docs.fivem.net/natives/?_0x2E89990DDFF670C3)
--- @param token number (int)
--- @param plateIndex number (int)
--- @return string
--- @overload fun(token: number, plateIndex: number): string
--- @deprecated
function N_0x2e89990ddff670c3(token, plateIndex) end

    
--- N_0x3001bef2feca3680
---
--- @hash [0x3001BEF2FECA3680](https://docs.fivem.net/natives/?_0x3001BEF2FECA3680)
---
--- @return boolean
--- @overload fun(): boolean
function N_0x3001bef2feca3680() end

    
--- ```
--- NativeDB Added Parameter 1: char* p0
--- NativeDB Added Parameter 2: char* p1
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0x33DF47CC0642061B](https://docs.fivem.net/natives/?_0x33DF47CC0642061B)
---
--- @return boolean
--- @overload fun(): boolean
function N_0x33df47cc0642061b() end

    
--- ```
--- Same as HAS_ACHIEVEMENT_BEEN_PASSED
--- ```
---
--- @hash [0x418DC16FAE452C1C](https://docs.fivem.net/natives/?_0x418DC16FAE452C1C)
--- @param achievement number (int)
--- @return boolean
--- @overload fun(achievement: number): boolean
function ScGetHasAchievementBeenPassed(achievement) end

    
--- # New Name: ScGetHasAchievementBeenPassed
--- ```
--- Same as HAS_ACHIEVEMENT_BEEN_PASSED
--- ```
---
--- @hash [0x418DC16FAE452C1C](https://docs.fivem.net/natives/?_0x418DC16FAE452C1C)
--- @param achievement number (int)
--- @return boolean
--- @overload fun(achievement: number): boolean
--- @deprecated
function N_0x418dc16fae452c1c(achievement) end

    
--- N_0x44aca259d67651db
---
--- @hash [0x44ACA259D67651DB](https://docs.fivem.net/natives/?_0x44ACA259D67651DB)
--- @param p0 any
--- @param p1 any
--- @return nil
--- @overload fun(p1: any): nil, any
function N_0x44aca259d67651db(p0, p1) end

    
--- ```
--- NativeDB Added Parameter 1: char* p0
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0x450819D8CF90C416](https://docs.fivem.net/natives/?_0x450819D8CF90C416)
---
--- @return boolean
--- @overload fun(): boolean
function N_0x450819d8cf90c416() end

    
--- N_0x4737980e8a283806
---
--- @hash [0x4737980E8A283806](https://docs.fivem.net/natives/?_0x4737980E8A283806)
--- @param p0 number (int)
--- @param p1 any
--- @return boolean
--- @overload fun(p0: number): boolean, any
function N_0x4737980e8a283806(p0, p1) end

    
--- N_0x487912fd248efddf
---
--- @hash [0x487912FD248EFDDF](https://docs.fivem.net/natives/?_0x487912FD248EFDDF)
--- @param p0 any
--- @param p1 number (float)
--- @return boolean
--- @overload fun(p0: any, p1: number): boolean
function N_0x487912fd248efddf(p0, p1) end

    
--- N_0x4a7d6e727f941747
---
--- @hash [0x4A7D6E727F941747](https://docs.fivem.net/natives/?_0x4A7D6E727F941747)
--- @param p0 any
--- @return any
--- @overload fun(): any, any
function N_0x4a7d6e727f941747(p0) end

    
--- ```
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0x4ED9C8D6DA297639](https://docs.fivem.net/natives/?_0x4ED9C8D6DA297639)
---
--- @return any
--- @overload fun(): any
function N_0x4ed9c8d6da297639() end

    
--- ```
--- NativeDB Added Parameter 1: char* p0
--- NativeDB Added Parameter 2: float* p1
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0x50A8A36201DBF83E](https://docs.fivem.net/natives/?_0x50A8A36201DBF83E)
---
--- @return boolean
--- @overload fun(): boolean
function N_0x50a8a36201dbf83e() end

    
--- ScEmailMessageClearRecipList
---
--- @hash [0x55DF6DB45179236E](https://docs.fivem.net/natives/?_0x55DF6DB45179236E)
---
--- @return nil
--- @overload fun(): nil
function ScEmailMessageClearRecipList() end

    
--- # New Name: ScEmailMessageClearRecipList
--- ScEmailMessageClearRecipList
---
--- @hash [0x55DF6DB45179236E](https://docs.fivem.net/natives/?_0x55DF6DB45179236E)
---
--- @return nil
--- @overload fun(): nil
--- @deprecated
function N_0x55df6db45179236e() end

    
--- Gets the status of the is valid license plate request
---
--- @hash [0x5C4EBFFA98BDB41C](https://docs.fivem.net/natives/?_0x5C4EBFFA98BDB41C)
--- @param token number (int)
--- @return number
--- @overload fun(token: number): number
function ScLicenseplateGetIsvalidStatus(token) end

    
--- # New Name: ScLicenseplateGetIsvalidStatus
--- Gets the status of the is valid license plate request
---
--- @hash [0x5C4EBFFA98BDB41C](https://docs.fivem.net/natives/?_0x5C4EBFFA98BDB41C)
--- @param token number (int)
--- @return number
--- @overload fun(token: number): number
--- @deprecated
function N_0x5c4ebffa98bdb41c(token) end

    
--- N_0x675721c9f644d161
---
--- @hash [0x675721C9F644D161](https://docs.fivem.net/natives/?_0x675721C9F644D161)
---
--- @return nil
--- @overload fun(): nil
function N_0x675721c9f644d161() end

    
--- N_0x699e4a5c8c893a18
---
--- @hash [0x699E4A5C8C893A18](https://docs.fivem.net/natives/?_0x699E4A5C8C893A18)
--- @param p0 number (int)
--- @param p1 string (char*)
--- @param p2 any
--- @return boolean
--- @overload fun(p0: number): boolean, string): boolean, any
function N_0x699e4a5c8c893a18(p0, p1, p2) end

    
--- ScInboxMessageGetUgcdata
---
--- @hash [0x69D82604A1A5A254](https://docs.fivem.net/natives/?_0x69D82604A1A5A254)
--- @param p0 any
--- @param p1 any
--- @return boolean
--- @overload fun(p0: any): boolean, any
function ScInboxMessageGetUgcdata(p0, p1) end

    
--- ScInboxMessageSendBountyPresenceEvent
---
--- @hash [0x6AFD2CD753FEEF83](https://docs.fivem.net/natives/?_0x6AFD2CD753FEEF83)
--- @param data string (char*)
--- @return boolean
--- @overload fun(data: string): boolean
function ScInboxMessageSendBountyPresenceEvent(data) end

    
--- # New Name: ScInboxMessageSendBountyPresenceEvent
--- ScInboxMessageSendBountyPresenceEvent
---
--- @hash [0x6AFD2CD753FEEF83](https://docs.fivem.net/natives/?_0x6AFD2CD753FEEF83)
--- @param data string (char*)
--- @return boolean
--- @overload fun(data: string): boolean
--- @deprecated
function N_0x6afd2cd753feef83(data) end

    
--- N_0x6bfb12ce158e3dd4
---
--- @hash [0x6BFB12CE158E3DD4](https://docs.fivem.net/natives/?_0x6BFB12CE158E3DD4)
--- @param p0 any
--- @return boolean
--- @overload fun(p0: any): boolean
function N_0x6bfb12ce158e3dd4(p0) end

    
--- Gets number of plates in the response of the get license plates request.
--- Range: \[0, count) can be used as second argument to \_0x1D4446A62D35B0D0 and \_0x2E89990DDFF670C3
---
--- @hash [0x700569DBA175A77C](https://docs.fivem.net/natives/?_0x700569DBA175A77C)
--- @param token number (int)
--- @return number
--- @overload fun(token: number): number
function ScLicenseplateGetCount(token) end

    
--- # New Name: ScLicenseplateGetCount
--- Gets number of plates in the response of the get license plates request.
--- Range: \[0, count) can be used as second argument to \_0x1D4446A62D35B0D0 and \_0x2E89990DDFF670C3
---
--- @hash [0x700569DBA175A77C](https://docs.fivem.net/natives/?_0x700569DBA175A77C)
--- @param token number (int)
--- @return number
--- @overload fun(token: number): number
--- @deprecated
function N_0x700569dba175a77c(token) end

    
--- ```
--- NativeDB Added Parameter 1: char* p0
--- NativeDB Added Parameter 2: int* p1
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0x710BCDA8071EDED1](https://docs.fivem.net/natives/?_0x710BCDA8071EDED1)
---
--- @return boolean
--- @overload fun(): boolean
function N_0x710bcda8071eded1() end

    
--- Starts a task to check an entered string for profanity on the ROS/Social Club services.
--- 
--- See also: [`SC_PROFANITY_GET_CHECK_IS_VALID`](https://docs.fivem.net/natives/?_0x1753344C770358AE) and [`SC_PROFANITY_GET_CHECK_IS_PENDING`](https://docs.fivem.net/natives/?_0x82E4A58BABC15AE7).
---
--- @hash [0x75632C5ECD7ED843](https://docs.fivem.net/natives/?_0x75632C5ECD7ED843)
--- @param string string (char*)
--- @param token number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
function ScProfanityCheckString(string, token) end

    
--- # New Name: ScProfanityCheckString
--- Starts a task to check an entered string for profanity on the ROS/Social Club services.
--- 
--- See also: [`SC_PROFANITY_GET_CHECK_IS_VALID`](https://docs.fivem.net/natives/?_0x1753344C770358AE) and [`SC_PROFANITY_GET_CHECK_IS_PENDING`](https://docs.fivem.net/natives/?_0x82E4A58BABC15AE7).
---
--- @hash [0x75632C5ECD7ED843](https://docs.fivem.net/natives/?_0x75632C5ECD7ED843)
--- @param string string (char*)
--- @param token number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
--- @deprecated
function N_0x75632c5ecd7ed843(string, token) end

    
--- # New Name: ScProfanityCheckString
--- Starts a task to check an entered string for profanity on the ROS/Social Club services.
--- 
--- See also: [`SC_PROFANITY_GET_CHECK_IS_VALID`](https://docs.fivem.net/natives/?_0x1753344C770358AE) and [`SC_PROFANITY_GET_CHECK_IS_PENDING`](https://docs.fivem.net/natives/?_0x82E4A58BABC15AE7).
---
--- @hash [0x75632C5ECD7ED843](https://docs.fivem.net/natives/?_0x75632C5ECD7ED843)
--- @param string string (char*)
--- @param token number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
--- @deprecated
function ScStartCheckStringTask(string, token) end

    
--- ScInboxMessageGetDataString
---
--- @hash [0x7572EF42FC6A9B6D](https://docs.fivem.net/natives/?_0x7572EF42FC6A9B6D)
--- @param p0 number (int)
--- @param context string (char*)
--- @param out string (char*)
--- @return boolean
--- @overload fun(p0: number, context: string, out: string): boolean
function ScInboxMessageGetDataString(p0, context, out) end

    
--- N_0x7db18ca8cad5b098
---
--- @hash [0x7DB18CA8CAD5B098](https://docs.fivem.net/natives/?_0x7DB18CA8CAD5B098)
---
--- @return any
--- @overload fun(): any
function N_0x7db18ca8cad5b098() end

    
--- ```
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0x7FFCBFEE44ECFABF](https://docs.fivem.net/natives/?_0x7FFCBFEE44ECFABF)
---
--- @return any
--- @overload fun(): any
function N_0x7ffcbfee44ecfabf() end

    
--- Returns a status for adding the license plate
---
--- @hash [0x8147FFF6A718E1AD](https://docs.fivem.net/natives/?_0x8147FFF6A718E1AD)
--- @param token number (int)
--- @return number
--- @overload fun(token: number): number
function ScLicenseplateGetAddStatus(token) end

    
--- # New Name: ScLicenseplateGetAddStatus
--- Returns a status for adding the license plate
---
--- @hash [0x8147FFF6A718E1AD](https://docs.fivem.net/natives/?_0x8147FFF6A718E1AD)
--- @param token number (int)
--- @return number
--- @overload fun(token: number): number
--- @deprecated
function N_0x8147fff6a718e1ad(token) end

    
--- ScProfanityGetCheckIsPending
---
--- @hash [0x82E4A58BABC15AE7](https://docs.fivem.net/natives/?_0x82E4A58BABC15AE7)
--- @param token number (int)
--- @return boolean
--- @overload fun(token: number): boolean
function ScProfanityGetCheckIsPending(token) end

    
--- # New Name: ScProfanityGetCheckIsPending
--- ScProfanityGetCheckIsPending
---
--- @hash [0x82E4A58BABC15AE7](https://docs.fivem.net/natives/?_0x82E4A58BABC15AE7)
--- @param token number (int)
--- @return boolean
--- @overload fun(token: number): boolean
--- @deprecated
function N_0x82e4a58babc15ae7(token) end

    
--- # New Name: ScProfanityGetCheckIsPending
--- ScProfanityGetCheckIsPending
---
--- @hash [0x82E4A58BABC15AE7](https://docs.fivem.net/natives/?_0x82E4A58BABC15AE7)
--- @param token number (int)
--- @return boolean
--- @overload fun(token: number): boolean
--- @deprecated
function ScGetCheckStringStatus(token) end

    
--- ```
--- sfink: from scripts:
--- func_720(socialclub::_0x8416FE4E4629D7D7("bIgnoreCheaterOverride"));
--- func_719(socialclub::_0x8416FE4E4629D7D7("bIgnoreBadSportOverride"));
--- ```
---
--- @hash [0x8416FE4E4629D7D7](https://docs.fivem.net/natives/?_0x8416FE4E4629D7D7)
--- @param name string (char*)
--- @return boolean
--- @overload fun(name: string): boolean
function ScGamerdataGetBool(name) end

    
--- # New Name: ScGamerdataGetBool
--- ```
--- sfink: from scripts:
--- func_720(socialclub::_0x8416FE4E4629D7D7("bIgnoreCheaterOverride"));
--- func_719(socialclub::_0x8416FE4E4629D7D7("bIgnoreBadSportOverride"));
--- ```
---
--- @hash [0x8416FE4E4629D7D7](https://docs.fivem.net/natives/?_0x8416FE4E4629D7D7)
--- @param name string (char*)
--- @return boolean
--- @overload fun(name: string): boolean
--- @deprecated
function N_0x8416fe4e4629d7d7(name) end

    
--- # New Name: ScGamerdataGetBool
--- ```
--- sfink: from scripts:
--- func_720(socialclub::_0x8416FE4E4629D7D7("bIgnoreCheaterOverride"));
--- func_719(socialclub::_0x8416FE4E4629D7D7("bIgnoreBadSportOverride"));
--- ```
---
--- @hash [0x8416FE4E4629D7D7](https://docs.fivem.net/natives/?_0x8416FE4E4629D7D7)
--- @param name string (char*)
--- @return boolean
--- @overload fun(name: string): boolean
--- @deprecated
function ScGetIsProfileAttributeSet(name) end

    
--- ScProfanityGetStringPassed
---
--- @hash [0x85535ACF97FC0969](https://docs.fivem.net/natives/?_0x85535ACF97FC0969)
--- @param token number (int)
--- @return boolean
--- @overload fun(token: number): boolean
function ScProfanityGetStringPassed(token) end

    
--- # New Name: ScProfanityGetStringPassed
--- ScProfanityGetStringPassed
---
--- @hash [0x85535ACF97FC0969](https://docs.fivem.net/natives/?_0x85535ACF97FC0969)
--- @param token number (int)
--- @return boolean
--- @overload fun(token: number): boolean
--- @deprecated
function N_0x85535acf97fc0969(token) end

    
--- ScInboxMessageGetBountyData
---
--- @hash [0x87E0052F08BD64E6](https://docs.fivem.net/natives/?_0x87E0052F08BD64E6)
--- @param index number (int)
--- @param outData number (int*)
--- @return boolean
--- @overload fun(index: number): boolean, number
function ScInboxMessageGetBountyData(index, outData) end

    
--- # New Name: ScInboxMessageGetBountyData
--- ScInboxMessageGetBountyData
---
--- @hash [0x87E0052F08BD64E6](https://docs.fivem.net/natives/?_0x87E0052F08BD64E6)
--- @param index number (int)
--- @param outData number (int*)
--- @return boolean
--- @overload fun(index: number): boolean, number
--- @deprecated
function N_0x87e0052f08bd64e6(index, outData) end

    
--- ```
--- NativeDB Introduced: v1290
--- ```
---
--- @hash [0x8A4416C0DB05FA66](https://docs.fivem.net/natives/?_0x8A4416C0DB05FA66)
--- @param newsStoryData any
--- @return boolean
--- @overload fun(): boolean, any
function N_0x8a4416c0db05fa66(newsStoryData) end

    
--- N_0x8cc469ab4d349b7c
---
--- @hash [0x8CC469AB4D349B7C](https://docs.fivem.net/natives/?_0x8CC469AB4D349B7C)
--- @param p0 number (int)
--- @param p1 string (char*)
--- @param p2 any
--- @return boolean
--- @overload fun(p0: number): boolean, string): boolean, any
function N_0x8cc469ab4d349b7c(p0, p1, p2) end

    
--- N_0x9237e334f6e43156
---
--- @hash [0x9237E334F6E43156](https://docs.fivem.net/natives/?_0x9237E334F6E43156)
--- @param p0 any
--- @return any
--- @overload fun(p0: any): any
function N_0x9237e334f6e43156(p0) end

    
--- N_0x92da6e70ef249bd1
---
--- @hash [0x92DA6E70EF249BD1](https://docs.fivem.net/natives/?_0x92DA6E70EF249BD1)
--- @param p0 string (char*)
--- @param p1 number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
function N_0x92da6e70ef249bd1(p0, p1) end

    
--- ScInboxGetMessageIsReadAtIndex
---
--- @hash [0x93028F1DB42BFD08](https://docs.fivem.net/natives/?_0x93028F1DB42BFD08)
--- @param msgIndex number (int)
--- @return boolean
--- @overload fun(msgIndex: number): boolean
function ScInboxGetMessageIsReadAtIndex(msgIndex) end

    
--- # New Name: ScInboxGetMessageIsReadAtIndex
--- ScInboxGetMessageIsReadAtIndex
---
--- @hash [0x93028F1DB42BFD08](https://docs.fivem.net/natives/?_0x93028F1DB42BFD08)
--- @param msgIndex number (int)
--- @return boolean
--- @overload fun(msgIndex: number): boolean
--- @deprecated
function N_0x93028f1db42bfd08(msgIndex) end

    
--- # New Name: ScInboxGetMessageIsReadAtIndex
--- ScInboxGetMessageIsReadAtIndex
---
--- @hash [0x93028F1DB42BFD08](https://docs.fivem.net/natives/?_0x93028F1DB42BFD08)
--- @param msgIndex number (int)
--- @return boolean
--- @overload fun(msgIndex: number): boolean
--- @deprecated
function IsScInboxValid(msgIndex) end

    
--- ScProfanityGetStringStatus
---
--- @hash [0x930DE22F07B1CCE3](https://docs.fivem.net/natives/?_0x930DE22F07B1CCE3)
--- @param token number (int)
--- @return number
--- @overload fun(token: number): number
function ScProfanityGetStringStatus(token) end

    
--- # New Name: ScProfanityGetStringStatus
--- ScProfanityGetStringStatus
---
--- @hash [0x930DE22F07B1CCE3](https://docs.fivem.net/natives/?_0x930DE22F07B1CCE3)
--- @param token number (int)
--- @return number
--- @overload fun(token: number): number
--- @deprecated
function N_0x930de22f07b1cce3(token) end

    
--- ScInboxMessageDoApply
---
--- @hash [0x9A2C8064B6C1E41A](https://docs.fivem.net/natives/?_0x9A2C8064B6C1E41A)
--- @param p0 number (int)
--- @return boolean
--- @overload fun(p0: number): boolean
function ScInboxMessageDoApply(p0) end

    
--- # New Name: ScInboxMessageDoApply
--- ScInboxMessageDoApply
---
--- @hash [0x9A2C8064B6C1E41A](https://docs.fivem.net/natives/?_0x9A2C8064B6C1E41A)
--- @param p0 number (int)
--- @return boolean
--- @overload fun(p0: number): boolean
--- @deprecated
function N_0x9a2c8064b6c1e41a(p0) end

    
--- # New Name: ScInboxMessageDoApply
--- ScInboxMessageDoApply
---
--- @hash [0x9A2C8064B6C1E41A](https://docs.fivem.net/natives/?_0x9A2C8064B6C1E41A)
--- @param p0 number (int)
--- @return boolean
--- @overload fun(p0: number): boolean
--- @deprecated
function ScInboxMessagePush(p0) end

    
--- ```
--- NativeDB Added Parameter 1: char* p0
--- NativeDB Added Parameter 2: char* p1
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0x9DE5D2F723575ED0](https://docs.fivem.net/natives/?_0x9DE5D2F723575ED0)
---
--- @return boolean
--- @overload fun(): boolean
function N_0x9de5d2f723575ed0() end

    
--- ScInboxMessageGetDataInt
---
--- @hash [0xA00EFE4082C4056E](https://docs.fivem.net/natives/?_0xA00EFE4082C4056E)
--- @param p0 number (int)
--- @param context string (char*)
--- @param out number (int*)
--- @return boolean
--- @overload fun(p0: number): boolean, string): boolean, number
function ScInboxMessageGetDataInt(p0, context, out) end

    
--- ```
--- NativeDB Added Parameter 1: int p0
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0xA468E0BE12B12C70](https://docs.fivem.net/natives/?_0xA468E0BE12B12C70)
---
--- @return boolean
--- @overload fun(): boolean
function N_0xa468e0be12b12c70() end

    
--- ScInboxMessageSendUgcStatUpdateEvent
---
--- @hash [0xA68D3D229F4F3B06](https://docs.fivem.net/natives/?_0xA68D3D229F4F3B06)
--- @param data string (char*)
--- @return nil
--- @overload fun(data: string): nil
function ScInboxMessageSendUgcStatUpdateEvent(data) end

    
--- # New Name: ScInboxMessageSendUgcStatUpdateEvent
--- ScInboxMessageSendUgcStatUpdateEvent
---
--- @hash [0xA68D3D229F4F3B06](https://docs.fivem.net/natives/?_0xA68D3D229F4F3B06)
--- @param data string (char*)
--- @return nil
--- @overload fun(data: string): nil
--- @deprecated
function N_0xa68d3d229f4f3b06(data) end

    
--- ```
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0xA770C8EEC6FB2AC5](https://docs.fivem.net/natives/?_0xA770C8EEC6FB2AC5)
--- @param name string (char*)
--- @param outValue number (float*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
function ScGamerdataGetFloat(name, outValue) end

    
--- # New Name: ScGamerdataGetFloat
--- ```
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0xA770C8EEC6FB2AC5](https://docs.fivem.net/natives/?_0xA770C8EEC6FB2AC5)
--- @param name string (char*)
--- @param outValue number (float*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
--- @deprecated
function N_0xa770c8eec6fb2ac5(name, outValue) end

    
--- ScInboxGetMessageTypeAtIndex
---
--- @hash [0xBB8EA16ECBC976C4](https://docs.fivem.net/natives/?_0xBB8EA16ECBC976C4)
--- @param msgIndex number (int)
--- @return Hash
--- @overload fun(msgIndex: number): Hash
function ScInboxGetMessageTypeAtIndex(msgIndex) end

    
--- # New Name: ScInboxGetMessageTypeAtIndex
--- ScInboxGetMessageTypeAtIndex
---
--- @hash [0xBB8EA16ECBC976C4](https://docs.fivem.net/natives/?_0xBB8EA16ECBC976C4)
--- @param msgIndex number (int)
--- @return Hash
--- @overload fun(msgIndex: number): Hash
--- @deprecated
function N_0xbb8ea16ecbc976c4(msgIndex) end

    
--- # New Name: ScInboxGetMessageTypeAtIndex
--- ScInboxGetMessageTypeAtIndex
---
--- @hash [0xBB8EA16ECBC976C4](https://docs.fivem.net/natives/?_0xBB8EA16ECBC976C4)
--- @param msgIndex number (int)
--- @return Hash
--- @overload fun(msgIndex: number): Hash
--- @deprecated
function ScInboxMessageInit(msgIndex) end

    
--- IsRockstarMessageReadyForScript
---
--- @hash [0xBC1CC91205EC8D6E](https://docs.fivem.net/natives/?_0xBC1CC91205EC8D6E)
---
--- @return boolean
--- @overload fun(): boolean
function IsRockstarMessageReadyForScript() end

    
--- # New Name: IsRockstarMessageReadyForScript
--- IsRockstarMessageReadyForScript
---
--- @hash [0xBC1CC91205EC8D6E](https://docs.fivem.net/natives/?_0xBC1CC91205EC8D6E)
---
--- @return boolean
--- @overload fun(): boolean
--- @deprecated
function N_0xbc1cc91205ec8d6e() end

    
--- SetHandleRockstarMessageViaScript
---
--- @hash [0xBFA0A56A817C6C7D](https://docs.fivem.net/natives/?_0xBFA0A56A817C6C7D)
--- @param toggle boolean
--- @return nil
--- @overload fun(toggle: boolean): nil
function SetHandleRockstarMessageViaScript(toggle) end

    
--- # New Name: SetHandleRockstarMessageViaScript
--- SetHandleRockstarMessageViaScript
---
--- @hash [0xBFA0A56A817C6C7D](https://docs.fivem.net/natives/?_0xBFA0A56A817C6C7D)
--- @param toggle boolean
--- @return nil
--- @overload fun(toggle: boolean): nil
--- @deprecated
function N_0xbfa0a56a817c6c7d(toggle) end

    
--- ```
--- NativeDB Added Parameter 1: char* p0
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0xC2C97EA97711D1AE](https://docs.fivem.net/natives/?_0xC2C97EA97711D1AE)
---
--- @return boolean
--- @overload fun(): boolean
function N_0xc2c97ea97711d1ae() end

    
--- ```
--- NativeDB Introduced: v323
--- NativeDB Added Parameter 1: Hash attrHash
--- NativeDB Added Parameter 2: float value
--- ```
---
--- @hash [0xC4C4575F62534A24](https://docs.fivem.net/natives/?_0xC4C4575F62534A24)
---
--- @return boolean
--- @overload fun(): boolean
function ScPresenceAttrSetFloat() end

    
--- # New Name: ScPresenceAttrSetFloat
--- ```
--- NativeDB Introduced: v323
--- NativeDB Added Parameter 1: Hash attrHash
--- NativeDB Added Parameter 2: float value
--- ```
---
--- @hash [0xC4C4575F62534A24](https://docs.fivem.net/natives/?_0xC4C4575F62534A24)
---
--- @return boolean
--- @overload fun(): boolean
--- @deprecated
function N_0xc4c4575f62534a24() end

    
--- ```
--- NativeDB Added Parameter 1: int p0
--- NativeDB Added Parameter 2: char* p1
--- NativeDB Added Parameter 3: float* p2
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0xC5A35C73B68F3C49](https://docs.fivem.net/natives/?_0xC5A35C73B68F3C49)
---
--- @return boolean
--- @overload fun(): boolean
function N_0xc5a35c73b68f3c49() end

    
--- ```
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0xC85A7127E7AD02AA](https://docs.fivem.net/natives/?_0xC85A7127E7AD02AA)
--- @param name string (char*)
--- @param outValue number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
function ScGamerdataGetInt(name, outValue) end

    
--- # New Name: ScGamerdataGetInt
--- ```
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0xC85A7127E7AD02AA](https://docs.fivem.net/natives/?_0xC85A7127E7AD02AA)
--- @param name string (char*)
--- @param outValue number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
--- @deprecated
function N_0xc85a7127e7ad02aa(name, outValue) end

    
--- Changes the license plate for the user (no insert).
---
--- @hash [0xD0EE05FE193646EA](https://docs.fivem.net/natives/?_0xD0EE05FE193646EA)
--- @param oldPlateText string (char*)
--- @param newPlateText string (char*)
--- @param plateData string (char*)
--- @return boolean
--- @overload fun(oldPlateText: string, newPlateText: string, plateData: string): boolean
function ScLicenseplateSetPlateData(oldPlateText, newPlateText, plateData) end

    
--- # New Name: ScLicenseplateSetPlateData
--- Changes the license plate for the user (no insert).
---
--- @hash [0xD0EE05FE193646EA](https://docs.fivem.net/natives/?_0xD0EE05FE193646EA)
--- @param oldPlateText string (char*)
--- @param newPlateText string (char*)
--- @param plateData string (char*)
--- @return boolean
--- @overload fun(oldPlateText: string, newPlateText: string, plateData: string): boolean
--- @deprecated
function N_0xd0ee05fe193646ea(oldPlateText, newPlateText, plateData) end

    
--- Checks if the "is valid license plate" request is still pending.
---
--- @hash [0xD302E99EDF0449CF](https://docs.fivem.net/natives/?_0xD302E99EDF0449CF)
--- @param token number (int)
--- @return boolean
--- @overload fun(token: number): boolean
function ScLicenseplateGetIsvalidIsPending(token) end

    
--- # New Name: ScLicenseplateGetIsvalidIsPending
--- Checks if the "is valid license plate" request is still pending.
---
--- @hash [0xD302E99EDF0449CF](https://docs.fivem.net/natives/?_0xD302E99EDF0449CF)
--- @param token number (int)
--- @return boolean
--- @overload fun(token: number): boolean
--- @deprecated
function N_0xd302e99edf0449cf(token) end

    
--- N_0xd8122c407663b995
---
--- @hash [0xD8122C407663B995](https://docs.fivem.net/natives/?_0xD8122C407663B995)
---
--- @return any
--- @overload fun(): any
function N_0xd8122c407663b995() end

    
--- ScInboxMessagePushGamerToEventRecipList
---
--- @hash [0xDA024BDBD600F44A](https://docs.fivem.net/natives/?_0xDA024BDBD600F44A)
--- @param networkHandle number (int*)
--- @return nil
--- @overload fun(): nil, number
function ScInboxMessagePushGamerToEventRecipList(networkHandle) end

    
--- # New Name: ScInboxMessagePushGamerToEventRecipList
--- ScInboxMessagePushGamerToEventRecipList
---
--- @hash [0xDA024BDBD600F44A](https://docs.fivem.net/natives/?_0xDA024BDBD600F44A)
--- @param networkHandle number (int*)
--- @return nil
--- @overload fun(): nil, number
--- @deprecated
function N_0xda024bdbd600f44a(networkHandle) end

    
--- RockstarMessageGetString
---
--- @hash [0xDF649C4E9AFDD788](https://docs.fivem.net/natives/?_0xDF649C4E9AFDD788)
---
--- @return string
--- @overload fun(): string
function RockstarMessageGetString() end

    
--- # New Name: RockstarMessageGetString
--- RockstarMessageGetString
---
--- @hash [0xDF649C4E9AFDD788](https://docs.fivem.net/natives/?_0xDF649C4E9AFDD788)
---
--- @return string
--- @overload fun(): string
--- @deprecated
function N_0xdf649c4e9afdd788() end

    
--- ```
--- NativeDB Introduced: v1290
--- ```
---
--- @hash [0xE4F6E8D07A2F0F51](https://docs.fivem.net/natives/?_0xE4F6E8D07A2F0F51)
--- @param p0 any
--- @return any
--- @overload fun(p0: any): any
function N_0xe4f6e8d07a2f0f51(p0) end

    
--- ```
--- NativeDB Added Parameter 1: char* p0
--- NativeDB Added Parameter 2: int* p1
--- NativeDB Added Parameter 3: char* p2
--- NativeDB Introduced: v323
--- ```
---
--- @hash [0xE75A4A2E5E316D86](https://docs.fivem.net/natives/?_0xE75A4A2E5E316D86)
---
--- @return boolean
--- @overload fun(): boolean
function N_0xe75a4a2e5e316d86() end

    
--- ```
--- NativeDB Introduced: v1290
--- ```
---
--- @hash [0xEA95C0853A27888E](https://docs.fivem.net/natives/?_0xEA95C0853A27888E)
---
--- @return nil
--- @overload fun(): nil
function N_0xea95c0853a27888e() end

    
--- ScProfanityCheckUgcString
---
--- @hash [0xEB2BF817463DFA28](https://docs.fivem.net/natives/?_0xEB2BF817463DFA28)
--- @param string string (char*)
--- @param token number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
function ScProfanityCheckUgcString(string, token) end

    
--- # New Name: ScProfanityCheckUgcString
--- ScProfanityCheckUgcString
---
--- @hash [0xEB2BF817463DFA28](https://docs.fivem.net/natives/?_0xEB2BF817463DFA28)
--- @param string string (char*)
--- @param token number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
--- @deprecated
function N_0xeb2bf817463dfa28(string, token) end

    
--- N_0xf22ca0fd74b80e7a
---
--- @hash [0xF22CA0FD74B80E7A](https://docs.fivem.net/natives/?_0xF22CA0FD74B80E7A)
--- @param p0 any
--- @return boolean
--- @overload fun(p0: any): boolean
function N_0xf22ca0fd74b80e7a(p0) end

    
--- ScInboxMessageGetString
---
--- @hash [0xF3E31D16CBDCB304](https://docs.fivem.net/natives/?_0xF3E31D16CBDCB304)
--- @param p0 number (int)
--- @return string
--- @overload fun(p0: number): string
function ScInboxMessageGetString(p0) end

    
--- # New Name: ScInboxMessageGetString
--- ScInboxMessageGetString
---
--- @hash [0xF3E31D16CBDCB304](https://docs.fivem.net/natives/?_0xF3E31D16CBDCB304)
--- @param p0 number (int)
--- @return string
--- @overload fun(p0: number): string
--- @deprecated
function N_0xf3e31d16cbdcb304(p0) end

    
--- N_0xf6baaaf762e1bf40
---
--- @hash [0xF6BAAAF762E1BF40](https://docs.fivem.net/natives/?_0xF6BAAAF762E1BF40)
--- @param p0 string (char*)
--- @param p1 number (int*)
--- @return boolean
--- @overload fun(): boolean, string): boolean, number
function N_0xf6baaaf762e1bf40(p0, p1) end

    
--- N_0xfe4c1d0d3b9cc17e
---
--- @hash [0xFE4C1D0D3B9CC17E](https://docs.fivem.net/natives/?_0xFE4C1D0D3B9CC17E)
--- @param p0 any
--- @param p1 boolean
--- @return boolean
--- @overload fun(p0: any, p1: boolean): boolean
function N_0xfe4c1d0d3b9cc17e(p0, p1) end

    
--- N_0xff8f3a92b75ed67a
---
--- @hash [0xFF8F3A92B75ED67A](https://docs.fivem.net/natives/?_0xFF8F3A92B75ED67A)
---
--- @return any
--- @overload fun(): any
function N_0xff8f3a92b75ed67a() end

    
--- ScInboxMessageGetDataBool
---
--- @hash [0xFFE5C16F402D851D](https://docs.fivem.net/natives/?_0xFFE5C16F402D851D)
--- @param p0 number (int)
--- @param p1 string (char*)
--- @return boolean
--- @overload fun(p0: number, p1: string): boolean
function ScInboxMessageGetDataBool(p0, p1) end

    
--- # New Name: ScInboxMessageGetDataBool
--- ScInboxMessageGetDataBool
---
--- @hash [0xFFE5C16F402D851D](https://docs.fivem.net/natives/?_0xFFE5C16F402D851D)
--- @param p0 number (int)
--- @param p1 string (char*)
--- @return boolean
--- @overload fun(p0: number, p1: string): boolean
--- @deprecated
function N_0xffe5c16f402d851d(p0, p1) end

    